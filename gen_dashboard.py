...
canvas{position:fixed;top:0;left:0;z-index:0;pointer-events:none;opacity:.18}
.wrap{position:relative;z-index:1;max-width:1500px;margin:0 auto;padding:20px 16px}
pre.ascii{color:#f5d020;font-size:clamp(6px,1.1vw,13px);line-height:1.2;text-align:center;
          margin-bottom:6px;text-shadow:0 0 20px #f5d02055}
.tag{text-align:center;color:#6a5010;font-size:10px;letter-spacing:6px;margin-bottom:18px;text-transform:uppercase}
.sr{display:flex;align-items:center;justify-content:center;gap:14px;margin-bottom:22px;flex-wrap:wrap}
...
.price{font-size:26px;color:#f5d020;font-weight:bold;text-shadow:0 0 10px #f5d02033}
.chg-up{color:#4ade80;font-size:13px}
.chg-dn{color:#f87171;font-size:13px}
.chg-neu{color:#8a7020;font-size:13px}
.badge{padding:4px 14px;border-radius:4px;font-size:12px;letter-spacing:2px;font-weight:bold}
...
.STAY_OUT{display:none!important}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:12px;margin-bottom:12px}
.card{background:#0d0a00;border:1px solid #3a2800;border-radius:6px;padding:16px}
.ct{color:#c9a030;font-size:9px;letter-spacing:3px;text-transform:uppercase;
    margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #251a00}
.m{display:flex;justify-content:space-between;align-items:center;margin:6px 0;font-size:12px}
.k{color:#8a6a20}.v{color:#f0c030}.g{color:#4ade80}.r{color:#f87171}
.bw{background:#1a1200;border-radius:2px;height:5px;flex:1;margin:0 8px}
.bf{height:5px;border-radius:2px;transition:width .8s}
.bb{background:linear-gradient(90deg,#1a4a1a,#4ade80)}
.br{background:linear-gradient(90deg,#4a1a1a,#f87171)}
table{width:100%;border-collapse:collapse;font-size:11px}
th{color:#c9a030;padding:8px 5px;text-align:left;border-bottom:1px solid #251a00;
   font-size:9px;text-transform:uppercase;letter-spacing:1px}
td{padding:6px 5px;border-bottom:1px solid #120d00;color:#d4a830}
td.LONG{color:#4ade80;font-weight:bold}
td.SHORT{color:#f87171;font-weight:bold}
td.STAY_OUT{color:#3a2808}
.ni{color:#9a8020;font-size:10px;padding:4px 0;border-bottom:1px solid #120d00;line-height:1.4}
.ns{color:#4a3010;margin-right:6px}
#upd{color:#2a1800;font-size:9px;text-align:right;margin-bottom:6px}
.footer{text-align:center;color:#1a1000;font-size:9px;margin-top:20px;letter-spacing:2px}
.pbar{height:2px;background:#150f00;margin-bottom:10px;border-radius:1px;overflow:hidden}
...
<div class="sr">
  <div class="dot"></div>
  <span style="font-size:11px;color:#5a4010;letter-spacing:2px">LIVE</span>
...
<div class="card">
  ...
  <div style="color:#6a5010;font-size:10px;margin-top:8px;line-height:1.5;
              border-top:1px solid #1a1200;padding-top:6px" id="sreason">--</div>
</div>
<div class="card">
  ...
  <div class="ct" style="margin-top:12px">Technical</div>
  ...
  <div class="ct" style="margin-top:12px">Macro</div>
...
<div class="card">
  ...
  <div class="ct" style="margin-top:12px">FTMO Limits</div>
...
<div class="card" style="margin-bottom:12px">
  ...
  <div id="nb" style="max-height:130px;overflow-y:auto">
    <span style="color:#1a1200">No news yet</span>
  </div>
</div>
<div class="card">
  ...
  <table><thead><tr>
    ...
    <tr><td colspan="13" style="text-align:center;padding:20px;color:#1a1200">No signals yet</td></tr>
  </tbody></table>
</div>
...
setInterval(()=>{
  ...
  cx.fillStyle="#c9a03015";cx.font="13px monospace";
  dr.forEach((y,i)=>{
...
function fd(n,pre="$"){
  ...
  return pre+Number(n).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2});
}
...
function render(d){
  ...
  const dec=(l.decision||"STAY OUT"),dcs=dec.replace(/ /g,"_");
  const p=l.price;
  ...
  sv("sc",l.confidence!=null?l.confidence+"%":"0%");
  ...
  sv("sl",rp.contracts!=null?rp.contracts:"--");
  sv("sk",fd(rp.risk_usd));
  sv("sreason",l.reason||"--");
  const db=l.debate_summary||{};
  const bl=db.bull_conf!=null?db.bull_conf:50;
  const br2=db.bear_conf!=null?db.bear_conf:50;
  ...
  sv("bp",bl+"%");sv("rp",br2+"%");
  sv("dw",db.winner||"--");
  sv("dm",db.margin!=null?db.margin+"pts":"--");
  ...
  const mb=ma.bias||ma.macro_bias||"--";
  sv("mb",mb+(ma.confidence?" ("+ma.confidence+"%)":""));
  ...
  if(nb){
    ...
    if(ni.length){
      nb.innerHTML=ni.slice(0,15).map(n=>"<div class=\\'ni\\'><span class=\\'ns\\'>["+n.source+"]</span>"+n.title+"</div>").join("");
    }
  }
  const tb=document.getElementById("ht"),rows=h.slice(-50).reverse();
  if(!rows.length){tb.innerHTML="<tr><td colspan=\\'13\\' style=\\'text-align:center;padding:20px;color:#1a1200\\'>No signals yet</td></tr>";return;}
  tb.innerHTML=rows.map(row=>{
    const dc2=(row.decision||"STAY OUT").replace(/ /g,"_");
    ...
    return "<tr>"
      ...
      +"<td class=\\'"+dc2+"\\'>"+dc2.replace(/_/g," ")+"</td>"
      +"<td style=\\'color:#c9a030\\'>"+(row.confidence!=null?row.confidence:"--")+"%</td>"
      ...
      +"<td style=\\'color:#4a3808;font-size:10px\\'>"+(row.reason||"--").substring(0,30)+"</td>"
      +"</tr>";
  }).join("");
...
setInterval(()=>{
  cd--;const pf=document.getElementById("pf");
  ...
  if(cd<=0){cd=REFRESH;window.location.reload();}
    ...
    os.makedirs("docs",exist_ok=True); os.makedirs("dashboard",exist_ok=True)
