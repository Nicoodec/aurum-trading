...
canvas{position:fixed;top:0;left:0;z-index:0;pointer-events:none;opacity:.25}
.wrap{position:relative;z-index:1;max-width:1500px;margin:0 auto;padding:20px 16px}
pre.ascii{color:#f5d020;font-size:clamp(8px,1.5vw,18px);line-height:1.2;text-align:center;margin-bottom:8px;text-shadow:0 0 20px #f5d02066}
...
.price{font-size:24px;color:#f5d020;font-weight:bold;text-shadow:0 0 10px #f5d02044}
...
.STAY_OUT{background:#1a1500;color:#7a6010;border:1px solid #4a3800}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px;margin-bottom:14px}
.card{background:#0f0c00;border:1px solid #4a3800;border-radius:8px;padding:18px}
...
.bb{background:linear-gradient(90deg,#2e6e2e,#4ade80)}
.br{background:linear-gradient(90deg,#6e2e2e,#f87171)}
...
#upd{color:#4a3800;font-size:9px;text-align:right;margin-bottom:8px}
.footer{text-align:center;color:#2a2000;font-size:9px;margin-top:24px;letter-spacing:2px}
.pbar{height:2px;background:#1a1500;margin-bottom:12px;border-radius:1px;overflow:hidden}
.pfill{height:100%;background:linear-gradient(90deg,#4a3000,#c9a030);border-radius:1px;transition:width 1s linear}
.chg-up{color:#4ade80}.chg-dn{color:#f87171}.chg-neu{color:#8a7020}
</style></head><body>
<canvas id="c"></canvas>
<div class="wrap">
<pre class="ascii">
...
<p class="tag">Multi-Agent Gold Swing Trading &middot; AI Powered &middot; FTMO Demo $25K</p>
...
<div class="sr">
  ...
  <span id="xc" style="font-size:13px"></span>
...
<div class="card">
  ...
  <div class="m"><span class="k">ATR used</span><span class="v" id="satr">--</span></div>
...
<div class="card">
  ...
  <div class="ct" style="margin-top:14px">Technical & Macro</div>
...
<div class="card">
  ...
  <div class="m"><span class="k">Capital</span><span class="v" id="pc">--</span></div>
...
<div class="footer">AURUM &middot; nicoodec.github.io/aurum-trading &middot; FTMO DEMO $25K &middot; Multi-Agent AI</div>
</div>
<script>
...
setInterval(()=>{
  ...
  cx.fillStyle="#c9a03022";cx.font="13px monospace";
  dr.forEach((y,i)=>{
...
function render(d){
  ...
  const p=l.price;
  sv("xp","XAU/USD "+(p?Number(p).toLocaleString("en-US",{minimumFractionDigits:3}):"--"));
  const chg=l.change_pct,ce=document.getElementById("xc");
  if(ce){
    ce.textContent=chg?(chg>0?"+":"")+chg+"%":"";
    ce.className=chg>0?"chg-up":chg<0?"chg-dn":"chg-neu";
  }
  ...
  sv("satr",rp.atr_used?fd(rp.atr_used,"")+" usd":"--");
  sv("sreason",l.reason||"--");
  ...
  sv("dxy",fr.dxy?fr.dxy.toFixed(2):"--");
  sv("t10",fr.t10y?fr.t10y+"%":"--");
  ...
  sv("fdr",ftmo.daily_remaining?fd(ftmo.daily_remaining):"$1,250.00");
  sv("ftr",ftmo.total_remaining?fd(ftmo.total_remaining):"$2,500.00");
