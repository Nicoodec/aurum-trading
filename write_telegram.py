...
code = open("utils/telegram_alerts.py", "w", encoding="utf-8")
code.write("""import requests, os
...
def send(message):
    ...
    try:
        ...
        if r.status_code != 200:
            print("[telegram] error:", r.text[:100])
...
def alert_decision(cycle_data):
    ...
    news_line = ni[0]["title"][:60] if ni else "No news"
    if d == "STAY OUT":
        msg = (
            "AURUM -- STAY OUT\\n"
            "XAU/USD: " + str(p) + "\\n"
...
def alert_closed(position, exit_price):
    ...
    send(
        ...
        + str(position.get("entry")) + " -> " + str(exit_price) + "\\n"
        + "PnL: " + str(pnl)
    )
...
""")
code.close()
print("telegram_alerts.py written OK")
